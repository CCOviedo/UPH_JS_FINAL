...
const input = { "Adult": 2, "Child": 3, "Senior": 1 };

const calcularEntrada = (input) => {
  return Object.keys(input).reduce((acc, key) => {
    return acc += input[key] * Zoo.prices[key]
  }, 0);
}; // Total

const todoslosnombresdeanimales = (zoo) => {
  return zoo.animals.map((animal) => {
    return animal.name
  })
};

const todoslosresidentes = (zoo) => {
  return zoo.animals.reduce((acc, animal) => {
    return [...acc, ...animal.residents]
  },[]);
};

//console.log(todoslosnombresdeanimales(Zoo));
//console.log(todoslosresidentes(Zoo));

//console.log(calcularEntrada(input));

// Horario en un lenguaje coloquial

const calculaHorario = (zoo) => {
  return Object.keys(zoo.hours).reduce((acc, day) => {
    // acc[day] = (condicion) ? valor1 : valor2;
    // Nota: 9
    acc[day] = (zoo.hours[day].open === 0 && zoo.hours[day].close === 0) ?
                `CLOSED` : `Open from ${zoo.hours[day].open} am until ${zoo.hours[day].close} pm`
    /* OK, nota: 6
    if (zoo.hours[day].open === 0 && zoo.hours[day].close === 0){
      acc[day] = `CLOSED`;
    }else {
      acc[day] = `Open from ${zoo.hours[day].open} am until ${zoo.hours[day].close} pm`;
    }
    */
    return acc;
  }, {});
...
console.log(calculaHorario(Zoo));
